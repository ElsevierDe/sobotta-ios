...
- (float)calculateCentreWidth {
	...
	//split up multiple lines
	if([self.annotation.title rangeOfString:@"\n"].location != NSNotFound){
		NSArray* lines = [self.annotation.title componentsSeparatedByString:@"\n"];
        NSMutableArray* linestmp = [lines mutableCopy];
        [linestmp removeObjectAtIndex:0];
		self.annotation.title = [lines objectAtIndex:0];
		self.annotation.subtitle = [linestmp componentsJoinedByString:@" "];
	}
...
- (void)setFramePositionForPoint:(CGPoint)point {
	float positionX = ((self.mapView.contentSize.width / self.mapView.orignalSize.width) * self.annotation.point.x) + self.annotation.offset.x;
	float positionY = ((self.mapView.contentSize.height / self.mapView.orignalSize.height) * self.annotation.point.y) + self.annotation.offset.y;
    ...
    NSLog(@"position (%f, %f) - contentSize: %@ - originalSize: %@ - annotation pos: %@", round(x), round(y), NSStringFromCGSize(self.mapView.contentSize), NSStringFromCGSize(self.mapView.orignalSize), NSStringFromCGPoint(self.annotation.offset));
